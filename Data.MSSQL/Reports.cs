    ...
    public class Reports : Data.Interface.IReports
    {
        ...
        private List<Data.Model.ReceiveReports> ReceiveDataReaderToList(SqlDataReader dataReader)
        {
            ...
            while (dataReader.Read())
            {
                ...
                model.Unit = dataReader.GetInt32(5);
                model.Type = dataReader.GetInt32(6);
                model.Fresequence = dataReader.GetInt32(7);
        ...
        private List<Data.Model.WorkInjuryReports> WorkInjuryDataReaderToList(SqlDataReader dataReader)
        {
            List<Data.Model.WorkInjuryReports> List = new List<Data.Model.WorkInjuryReports>();
            Data.Model.WorkInjuryReports model = null;
            while (dataReader.Read())
            {
                model = new Data.Model.WorkInjuryReports();
                model.User_Name = dataReader.GetString(4);
...
// from 
//(select distinct temp.Type,temp.date1,temp.date2,temp.days,users.Name as username,Organize .Name as deptname,UsersInfo.LastPublicDate,UsersInfo.PublicDate ,UsersInfo.RestDate    from (select * from TempTest ) temp 
...
//group by deptname ,username,LastPublicDate,PublicDate,RestDate) last";
           ...
           string sql = @"select deptname,username,AffairDays,WeddingDays,SickDays,MaternityDays,ChaperonageDays,YearDays,HomeDays,
LastPublicDate,PublicDate,RestDate,
case when LastPublicDate-yeardays>0 then LastPublicDate-yeardays else 0 end as 'subLastPublicDate',
case when LastPublicDate-yeardays<0 and LastPublicDate+PublicDate-yeardays>0 then LastPublicDate+PublicDate-yeardays else 
   case  when LastPublicDate+PublicDate-yeardays<0 then 0 else PublicDate end  end as 'subPublicDate',
isnull(RestDate,0)-isnull(AdjustDays,0) as 'subRestDate'
from (select distinct  deptname,username,
SUM(CASE WHEN Type='事假' then Days else 0 end) as 'AffairDays',
SUM(CASE WHEN Type='婚假' then Days else 0 end) as 'WeddingDays',
SUM(CASE WHEN Type='病假' then Days else 0 end) as 'SickDays',
...
SUM(CASE WHEN Type='探亲假' then Days else 0 end) as 'HomeDays',
isnull(LastPublicDate,0) as LastPublicDate ,isnull(PublicDate,0) as PublicDate ,isnull(RestDate,0) as RestDate,
SUM(CASE WHEN Type='调休假' then Days else 0 end) as 'AdjustDays' 
 from 
(select distinct temp.Type,temp.date1,temp.date2,temp.days,users.Name as username,Organize .Name as deptname,UsersInfo.LastPublicDate,UsersInfo.PublicDate ,UsersInfo.RestDate    from (select * from TempTest ) temp 
inner join Users on substring(temp.UserID,3,LEN(temp.userid)) =convert(varchar(50),users.ID)
inner join Organize  on Temp.DeptID =convert(varchar(50),Organize .ID)
inner join (select * from workflowtask where status='2' and Comment='同意') task on  task.InstanceID=Convert(varchar(50),temp.id)
left join UsersInfo on users.id=usersinfo.userid where date1>='2016/01/01' and date2<='2016/01/31') as record
group by deptname ,username,LastPublicDate,PublicDate,RestDate) last";
...
group by deptname ,username,LastPublicDate,PublicDate,RestDate) last";
        ...
        public List<Data.Model.RestDetailReports> GetRestDetailReportAllPara(string starttime, string endtime)
        {
            ...
            string sql = @"select Organize .Name as deptname,users.Name as username,temp.Type,temp.days,temp.date1,temp.date2   from TempTest temp 
                inner join Users on substring(temp.UserID,3,LEN(temp.userid)) =convert(varchar(50),users.ID)
                inner join Organize  on Temp.DeptID =convert(varchar(50),Organize .ID)
        ...
        {
            string sql = @"select dictionary.title as name,record.begintime as begintime,record.endtime as endtime,record.Model as Model,record.Quantity as Quantity,
record.Unit as Unit,record.Type as Type,record.Fresequence as Fresequence
 from(
select name,min(SqDateTime) as begintime,max(SqDateTime) as endtime,sum(cast(Model as int)) as Model,sum(cast(Quantity as int)) as Quantity,
...
select name,cast('{0}' as datetime) as begintime,cast('{1}' as datetime) as endtime,sum(cast(Model as int)) as Model,sum(cast(Quantity as int)) as Quantity,
sum(cast(Unit as int)) as Unit,
...
dictionary on record.name=dictionary.id ";
        ...
        public List<Data.Model.WorkInjuryReports> GetWorkInjuryReportAllPara()
        {

            string sql = @"select * from dbo.PerformanceAttechItemList where Item_name='工伤'";
            sql = string.Format(sql);

            SqlDataReader dataReader = dbHelper.GetDataReader(sql);
            List<Data.Model.WorkInjuryReports> List = WorkInjuryDataReaderToList(dataReader);
            dataReader.Close();
            return List;
        }


        public List<Data.Model.WorkInjuryReports> GetWorkInjuryReportAllPara(string starttime, string endtime, string item_type)
        {

            string sql = @"select * from dbo.PerformanceAttechItemList where Item_name='{2}' and addtime>='{0}' and addtime<='{1}'";
            sql = string.Format(sql, starttime, endtime, item_type);
