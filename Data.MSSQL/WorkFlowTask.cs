...
namespace Data.MSSQL
{
    public class WorkFlowTask : Data.Interface.IWorkFlowTask
    {
        ...
        public List<Data.Model.WorkFlowTask> GetTasks(Guid userID, out string pager, string query="", string title="", string flowid="", string sender="", string date1="", string date2="", int type=0)
        {
            ...
            StringBuilder sql = new StringBuilder("SELECT *,ROW_NUMBER() OVER(ORDER BY " + (type == 0 ? "ReceiveTime DESC" : "CompletedTime1 DESC") + ") AS PagerAutoRowNumber FROM (select WorkFlowTask.* FROM WorkFlowTask inner join (select WorkFlow.ID,Dictionary.Title from WorkFlow inner join Dictionary on WorkFlow.Type=Dictionary.ID where  Dictionary.Title='办公类流程') wf on WorkFlowTask.FlowID=wf.ID) WorkFlowTask  WHERE  (StepName <> '分管领审批') and ReceiveID=@ReceiveID");
        ...
        /// <param name="type">0待办 1已完成</param>
        /// <returns></returns>
        public List<Data.Model.WorkFlowTask> GetBusinessTasks(Guid userID, out string pager, string query = "", string title = "", string flowid = "", string sender = "", string date1 = "", string date2 = "", int type = 0)
        {
            List<SqlParameter> parList = new List<SqlParameter>();
            StringBuilder sql = new StringBuilder("SELECT *,ROW_NUMBER() OVER(ORDER BY " + (type == 0 ? "ReceiveTime DESC" : "CompletedTime1 DESC") + ") AS PagerAutoRowNumber FROM (select * from (select WorkFlowTask.* FROM WorkFlowTask left join WorkFlow on WorkFlowTask.FlowID=WorkFlow.id inner join Dictionary on WorkFlow.Type=Dictionary.ID where Dictionary.Title='业务类流程') WorkFlowTask) WorkFlowTask  WHERE (StepName <> '分管领审批') and  ReceiveID=@ReceiveID");
            sql.Append(type == 0 ? " AND Status IN(0,1)" : " AND Status IN(2,3)");
            parList.Add(new SqlParameter("@ReceiveID", SqlDbType.UniqueIdentifier) { Value = userID });
            if (!title.IsNullOrEmpty())
            {
...
//                WHERE a.ID=(SELECT TOP 1 ID FROM WorkFlowTask WHERE FlowID=a.FlowID AND GroupID=a.GroupID ORDER BY Sort DESC)");
            ...
            StringBuilder sql = new StringBuilder(@"SELECT a.*,ROW_NUMBER() OVER(ORDER BY a.SenderTime DESC) AS PagerAutoRowNumber FROM WorkFlowTask a
                            WHERE a.ID=(SELECT TOP 1 ID FROM WorkFlowTask WHERE FlowID=a.FlowID AND GroupID=a.GroupID ORDER BY Sort,SenderTime )");
