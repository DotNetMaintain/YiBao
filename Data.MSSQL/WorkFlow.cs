...
namespace Data.MSSQL
{
    public class WorkFlow : Data.Interface.IWorkFlow
    {
        ...
        /// 查询流程类型所有ID和名称
        /// </summary>
        public Dictionary<Guid, string> GetTypeIDAndName(string type)
        {
            string sql = @"select ID,Name from (select wf.*,Dictionary.Title from (SELECT     ID, Name,Type
                            FROM         WorkFlow
                            WHERE     (Status < 4) ) wf
                            inner join Dictionary on wf.Type=Dictionary.id) dic
                            where Title='{0}'
                           ORDER BY Name";
